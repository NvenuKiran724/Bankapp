    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                if (response=="Yes")
                {
                    bool returnValue = ReadAndInsertCustomerData();
        ...
        {
            string filePath = ConfigurationManager.AppSettings["CustomerInputsFilePath"];
            ...
            try
            {
                using (StreamReader reader = new StreamReader(File.OpenRead(filePath)))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        string inputRecordLine = reader.ReadLine();
        ...
        private static CustomerRecord ValidateInputData(string recordData)
        {
            ...
            customerRecord.AccountOpeningDate = DateTime.Parse(values[7]);
